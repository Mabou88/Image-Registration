...
{
public:
    LC2_function(ImageType::Pointer im_Fixed, ImageType::Pointer im_Moving,string out,int type);
    ...
    const
    {
       ...
       //sizeAccessible[2] = sizeIm[2]-6;
		...
		int position=0;
		// while(!US_it.IsAtEnd())
		for (int xx=95;xx<170;xx++){
      // xx++;
	   //xx++;
        for (int yy=60;yy<145;yy++){
		//yy++;
		//yy++;
			for (int zz=67;zz<84;zz++){
			//zz++;
			//zz++;
