...
{
public:
    LC2_function(ImageType::Pointer im_Fixed, ImageType::Pointer im_Moving,string out,int type, ImageType::PointType centre,int coderegion);
    ...
    const
    {
       ...
       // ImageType::Pointer movedImage = TransformImage(params, 1);
		...
		ImageType::Pointer movedImage;
		MaskType::Pointer movedMask;

		//dépendamment si on fait euler ou affine

		if (typetransform==1){
            movedImage = TransformImage(params, 1);
			movedMask = TransformMask(params, 1);
        } 
		if(typetransform==2){
            movedImage = TransformImage(params, 2);
			movedMask = TransformMask(params, 2);
        } 
       ...
       //sizeAccessible[2] = sizeIm[2]-6;
		...
		//implemente les coordonnes de la ROI
		//choosecoordregion();
		cout<<" coord en x"<<coordregion_[0]<<coordregion_[1]<<endl;


		int position=0;
		// while(!US_it.IsAtEnd())
		for (int xx=coordregion_[0];xx<coordregion_[1];xx++){
       xx++;
	   //xx++;
        for (int yy=coordregion_[2];yy<coordregion_[3];yy++){
		yy++;
		//yy++;
			for (int zz=coordregion_[4];zz<coordregion_[5];zz++){
			zz++;
			//zz++;

					itk::Index<3> index_us={{xx,yy,zz}};
            ...
            //        //on ne considere le voisinage que si le centre appartient à la region blanche du mask et s'il est a l'int de l'im IRM accessible
                    ...
                    m_FixedImage->TransformIndexToPhysicalPoint(index_us,p);
            ...
            if(m_useLiverMask)
            {
               ...
               // {
					...
					// std::chrono::duration<double> time_span0 = std::chrono::duration_cast<std::chrono::duration<double>>(t10 - t1);

					//std::cout<<"temps de parcours pour remplir les matrices en s"<<time_span0.count()<<std::endl;
                 ...
                 //   std::chrono::high_resolution_clock::time_point t11 = std::chrono::high_resolution_clock::now();
					...
					// std::chrono::duration<double> time_span1 = std::chrono::duration_cast<std::chrono::duration<double>>(t11 - t10);

					//std::cout<<"temps de parcours pour calculé la transposé et faire la multiplication en s "<<time_span1.count()<<std::endl;
					...
					//	std::chrono::duration<double> time_span3 = std::chrono::duration_cast<std::chrono::duration<double>>(t13 - t11);

					//std::cout<<"temps de parcours pour calculer l'inverse en s: "<<time_span3.count()<<std::endl;
                     ...
                     //   std::chrono::high_resolution_clock::time_point t14 = std::chrono::high_resolution_clock::now();
						...
						//std::chrono::duration<double> time_span4 = std::chrono::duration_cast<std::chrono::duration<double>>(t14 - t13);

					//std::cout<<"temps de parcours pour calculer coefficient sans exception en s: "<<time_span4.count()<<std::endl;
                      ...
                      //  std::cerr<<e<<std::endl;
						...
						*/
						...
						// std::chrono::duration<double> time_span5 = std::chrono::duration_cast<std::chrono::duration<double>>(t17 - t16);

					//std::cout<<"temps de parcours calculer coefficient s'il y a exception en s"<<time_span5.count()<<std::endl;
                  ...
                  // std::chrono::high_resolution_clock::time_point t15 = std::chrono::high_resolution_clock::now(); 
					...
					//std::chrono::duration<double> time_span2 = std::chrono::duration_cast<std::chrono::duration<double>>(t15 - t10);

					//std::cout<<"temps de parcours pour faire les calculs s: "<<time_span2.count()<<std::endl;
			...
			//std::cout<<"temps de parcours pour un bloc en s: "<<time_span.count()<<std::endl;	
			
			//if(time_span.count()==0){
				//cout<<"LC2_du t0"<<lc2varsum2<<endl;
            ...
            else
            {
                ...
                if(int(mask_shrunk->GetPixel(index_us))==255)// && m_accessibleMRI->TransformPhysicalPointToIndex(p, i))
                {
		...
		//	std::chrono::high_resolution_clock::time_point t2 = std::chrono::high_resolution_clock::now();
			...
			//cout<<"LC2"<<lc2varsum2<<endl;
            
          //  ++US_it;
		//	++US_it;

			//cout<<"Index:"<<index_us<<endl;
