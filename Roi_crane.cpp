...
void Roi_crane::limiteparcercle(int z){
	...
	setlimitexy(z);
	itk::Index<3> test={{250,200,150}};
	roi_crane->SetPixel(test,0);
	...
	if(z<150){
		centre[0]=centre[0]+(150-z)/8;
		centre[1]=centre[1]+(150-z)/1.5;
	}
	...
	//pour le cercle
	int position=0;
	int limitesuperieur=140;
	int limiteinferieur=140;
	...
	if (z<150){
		longueur=longueur-(150-z)/1.1;
	}
	if (z<120){
		largueur=largueur-(120-z)/0.75;
	}
	...
	for (int i=lim_min_x;i<lim_max_x;i++){
		...
		for (int j=0;j<150;j++){
			...
			if (limite0==true && pixelvalue<limiteinferieur){
	...
	for (int j=lim_min_y;j<lim_max_y;j++){
		...
		for (int i=lim_max_x;i>centre[0];i--){
			...
			if (limite==true && pixelvalue<limiteinferieur){
...
void Roi_crane::regionexterieur(int z){
	...
	for (int j=0;j<dim_y;j++){
		...
		for (int i=dim_x;i>centre[0];i--){
		itk::Index<3> curseurread={{i,j,z}};
		ImageType::PixelType pixelvalue;
		pixelvalue=roi_crane->GetPixel(curseurread);
			if (pixelvalue==0){
				for (int ii=i;ii<dim_x;ii++){
					itk::Index<3> curseurwrite={{ii,j,z}};
					roi_crane->SetPixel(curseurwrite,0);
				}
			}
		...
		pixelvalue=roi_crane->GetPixel(curseurread);
			if (pixelvalue==0){
				for (int jj=j;jj>=0;jj--){
					itk::Index<3> curseurwrite={{i,jj,z}};
					roi_crane->SetPixel(curseurwrite,0);
				}
			}

		}
		for (int j=dim_y;j>centre[1];j--){
		itk::Index<3> curseurread={{i,j,z}};
		ImageType::PixelType pixelvalue;
		pixelvalue=roi_crane->GetPixel(curseurread);
			if (pixelvalue==0){
				for (int jj=j;jj<dim_y;jj++){
					itk::Index<3> curseurwrite={{i,jj,z}};
					roi_crane->SetPixel(curseurwrite,0);
				}
			}
