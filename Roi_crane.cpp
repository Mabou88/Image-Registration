...
int Roi_crane::getlargueur(){
	...
	int largueur=lim_max_y-lim_min_y;
	return largueur;
...
void Roi_crane::limiteparcercle(int z){
	...
	for (int p=0;p<position_a;p++){
		...
		if(z<148 && z>126){
			int longueur_ant=longueur/((148-z)*0.03+1);
		...
		if(z<90){
			rayon=(double((cranecercleav[p][0]-centre[0])*(cranecercleav[p][0]-centre[0]))/double(largueur/1.25*largueur/1.25)+double((cranecercleav[p][1]-centre[1])*(cranecercleav[p][1]-centre[1]))/double((longueur/4)*(longueur/4)));
			cout<<"rayon ant"<<rayon<<endl;
		}
