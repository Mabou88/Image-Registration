...
Roi_crane::Roi_crane(ImageType::Pointer itkimageus, int type ){
...
void Roi_crane::makeirmus(){
	if(type_==2){
		for (int i=64;i<116;i++){
			for (int j=80;j<140;j++){
				for (int k=90;k<120;k++){
					...
					if (pixelvalue>135){
						int r1=rand() % 5 -5;
						int d1=abs(centrei-i)*1.5;
						int pixel=125+r1+d1;
						if(pixel>170)
							pixel=170;
						roi_crane->SetPixel(curseur,pixel);
		...
		for (int i=118;i<150;i++){
			for (int j=55;j<90;j++){
				for (int k=65;k<80;k++){
					...
					pixelvalue=roi_crane->GetPixel(curseur);
					if (pixelvalue>150){
						int pixelvalue2=pixelvalue- 0.8*(pixelvalue-150)+rand() % 10 -5;
						roi_crane->SetPixel(curseur,pixelvalue2);
		...
		for (int j=0;j<dim_y;j++){
			for (int k=0;k<dim_z;k++){
				...
				if (pixelvalue>10){
					...
					//si cest significatif alors on prend les limites de manieres normales
