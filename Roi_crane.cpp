...
void Roi_crane::limiteparcercle(int z){
	...
	int limitesuperieur=200-(z-60)/2.5;
	int limiteinferieur=200-(z-60)/2.5;
	...
	for (int p=0;p<position_a;p++){
		...
		if(z<125 && z>99){
			rayon=(double((cranecercleav[p][0]-centre[0])*(cranecercleav[p][0]-centre[0]))/double(largueur*largueur)+double((cranecercleav[p][1]-centre[1])*(cranecercleav[p][1]-centre[1]))/double((longueur/1.75)*(longueur/1.75)));
			cout<<"rayon ant"<<rayon<<endl;
		}
		if(z<90){
			rayon=(double((cranecercleav[p][0]-centre[0])*(cranecercleav[p][0]-centre[0]))/double(largueur/1.25*largueur/1.25)+double((cranecercleav[p][1]-centre[1])*(cranecercleav[p][1]-centre[1]))/double((longueur/3)*(longueur/3)));
			cout<<"rayon ant"<<rayon<<endl;
		}
	...
	for (int i=lim_min_x;i<lim_max_x;i++){
		...
		for (int j=lim_max_y;j>limitepost_y;j--){
			...
			if (limitepost==true && pixelvalue<limiteinferieur){
	...
	for (int p=0;p<position;p++){
		...
		if (z<70){
			rayon=(double((cranecercle[p][0]-centre[0])*(cranecercle[p][0]-centre[0]))/double((largueur/1.5)*(largueur/1.5))+double((cranecercle[p][1]-centre[1])*(cranecercle[p][1]-centre[1]))/double((longueur/2)*(longueur/2)));
		}
		//cout<<"rayon"<<rayon<<endl;
		if (0.75<rayon && rayon<3){
...
void Roi_crane::regionexterieur(int z){
	...
	for (int j=0;j<dim_y;j++){
		...
		for (int i=lim_max_x;i>centre[0]*1.2;i--){
		itk::Index<3> curseurread={{i,j,z}};
		ImageType::PixelType pixelvalue;
		pixelvalue=roi_crane->GetPixel(curseurread);
			if (pixelvalue==0){
...
void Roi_crane::regioninferieur(){
	for (int i=0;i<dim_x;i++){
		for (int j=0;j<dim_y;j++){
			for (int z=0;z<60;z++){
