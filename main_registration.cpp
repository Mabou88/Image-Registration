...
//************************************************************************* 
  ...
  //*************************************************************************
	...
	//translate
		 ...
		 vecteurtranslation_z_scale=vecteurtranslation_z_scale*-20;
...
//**********************************************
   ...
   x_lower_affineT(0) = -1;
    ...
    //make sure that the mask is computed on US_Shrunk but that we use the rescaled image to compute LC2
    //LC2.setMovingImage(rescaled_US);
    LC2.setMaxRot(0.1); //0.3 for best initialisation
    LC2.setMaxTrans(5);//10
	LC2.setMaxScale(1.05); //valeur arbitraire a verifier
	//plus petit shear
	LC2.setMaxShear(0.1);
    LC2.setRadius(radius);
