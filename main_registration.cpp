...
//************************************************************************* 
	...
	int typeorientation=1; //1 P2, P5 v3 de base--- 2 v3 a lenvers P1
   ...
   //*************************************************************************	
//Pour rotater les ventricules segmentées si nécessaire
//************************************************************************* 
	BinaryReaderType::Pointer reader_us_v = BinaryReaderType::New();
    reader_us_v->SetImageIO(mask_io);
    reader_us_v->SetFileName("C:/Users/Marc-Antoine/Documents/Maitrise/Image/Image_US_mars2016P1-P13/Nifti_image_mars_2016/Patient1/P1V3-IM_0003ventriculedroit.nii");
    try {
        reader_us_v->Update();
    } catch (itk::ExceptionObject &e) {
        cout<<"Error while reading US mask"<<endl;
	...
	BinaryImageType::Pointer image_us_vseg=BinaryImageType::New();
	image_us_vseg=reader_us_v->GetOutput();

	 BinaryWriterType::Pointer   writerv0 =  BinaryWriterType::New();
		itk::NiftiImageIO::Pointer ioimageniftiv0=itk::NiftiImageIO::New();

		 string outputfilev0="C:/Users/Marc-Antoine/Documents/Imagecode/output/ventriculeP1V3rotUS0.nii";
		

		 writerv0->SetImageIO(ioimageniftiv0);
		writerv0->SetFileName( outputfilev0);
		writerv0->SetInput(image_us_vseg);
		 writerv0->Update();
	
	ResamplerBinaryType::Pointer resamplerv = ResamplerBinaryType::New();
    resamplerv->SetInput(image_us_vseg);
	resamplerv->SetTransform(rotation2);

	resamplerv->SetSize(image_us_vseg->GetLargestPossibleRegion().GetSize());
    resamplerv->SetOutputSpacing(image_us_vseg->GetSpacing());
   resamplerv->SetOutputDirection(image_us_vseg->GetDirection());
   resamplerv->SetOutputOrigin((image_us_vseg->GetOrigin()));
   resamplerv->Update();

   image_us_vseg = resamplerv->GetOutput();
    
		 BinaryWriterType::Pointer   writerv1 =  BinaryWriterType::New();
		itk::NiftiImageIO::Pointer ioimageniftiv1=itk::NiftiImageIO::New();

		 string outputfilev1="C:/Users/Marc-Antoine/Documents/Imagecode/output/ventriculeP1V3rotUS1.nii";
		

		 writerv1->SetImageIO(ioimageniftiv1);
		writerv1->SetFileName( outputfilev1);
		writerv1->SetInput(image_us_vseg);
		 writerv1->Update();
  ...
  //*************************************************************************
   ...
   ResamplerBinaryType::Pointer resamplerv2 = ResamplerBinaryType::New();
    resamplerv2->SetInput(image_us_vseg);
	resamplerv2->SetTransform(rotation3);

	resamplerv2->SetSize(image_us_vseg->GetLargestPossibleRegion().GetSize());
    resamplerv2->SetOutputSpacing(image_us_vseg->GetSpacing());
   resamplerv2->SetOutputDirection(image_us_vseg->GetDirection());
   resamplerv2->SetOutputOrigin((image_us_vseg->GetOrigin()));
   resamplerv2->Update();

   image_us_vseg = resamplerv->GetOutput();
    

		 BinaryWriterType::Pointer   writerv =  BinaryWriterType::New();
		itk::NiftiImageIO::Pointer ioimageniftiv=itk::NiftiImageIO::New();

		 string outputfilev="C:/Users/Marc-Antoine/Documents/Imagecode/output/ventriculeP1V3rotUS.nii";
		

		 writerv->SetImageIO(ioimageniftiv);
		writerv->SetFileName( outputfilev);
		writerv->SetInput(image_us_vseg);
		 writerv->Update();




   //selection image IRM avec masque
...
Roi_crane volumeirm2(rescaled_IRM,2);
	...
	//volumeirm2.makeirmus();
	//rescaled_IRM=volumeirm2.getcraneROI();
...
//modification du contraste de l'image IRM
//*************************************************************************
 //  Roi_crane volumeirm3(rescaled_IRM,2);
	//volumeirm3.makeirmus();
	//rescaled_IRM=volumeirm3.getcraneROI();

	//*************************************************************************	

//modification du contraste de l'image US
//*************************************************************************
  // Roi_crane volumeus3(US_shrunk,1);
	//volumeus3.makeusirm();
	//US_shrunk=volumeus3.getcraneROI();
...
//    final parameters : [-0.004576003822187878, -0.018662112915280166, 0.021674120903660443, 1.335316780176604, 0.012001995175122276, -0.08281897027573802]
  ...
  for (int j=1;j<6;j++){
    ...
    double m = 13;
    //ressayer en changeant le diviser par radius
    //rho begin
    double radius = 0.9; //0.9 //zero makes no sense !!! essayer 0.1 et 0.01 comme combinaison avec precision
	...
	int type=1;
		...
		//reset l'indice et change de region
		int indice=0;
		//if(i>0){
			//coderegion=2;
			//indice=i-1;
		...
		double rot_init=0.2/(1+indice*0.4);
		double trans_init=5/(1+indice*0.4);
	...
	precision=0.02*j;
	//changer le radius dans bobyqa
    
    if(useLiverMask) LC2.setLiverMask(LiverMask);
    
	//double best_score=1;
   double best_score = find_min_bobyqa(LC2, initialParameters, m, x_lower, x_upper, rad, precision, nombreIteration);
...
//**********************************************
	...
	string nomiteration0="iterradx_precx_";
	string Result1;          
	ostringstream convert1;  
	convert1 << precision;      
	Result1 = convert1.str();
	nomiteration0.replace(7,1,Result0);
	nomiteration0.replace(15,1,Result1);

	//string Result1;          
	//ostringstream convert1;  
	///convert1 << j;      
	//Result1 = convert1.str();
	//string nomiteration1="iterationprecx_";
	//nomiteration0.replace(13,1,Result1);



    string outP =outputPath+nomiteration0+"parameters.txt";
	...
	//convert << i;      
	//Result = convert.str();
	string nomiteration=nomiteration0+ "finalregisUS.nii.gz";
   ...
   x_lower_affineT(0) = -1;
	...
	double best_score_af = find_min_bobyqa(LC2_af, initialParameters_affineT, m_af, x_lower_affineT, x_upper_affineT, radaf, precision_af, nombreIteration_af);
    //double best_score = find_min_bobyqa(LC2_function(rescaled_IRM, US_shrunk), initialParameters, m, x_lower, x_upper, radius, precision, nombreIteration);
